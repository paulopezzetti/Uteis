...
Get-VIEvent -maxsamples 10000 -Start (Get-Date).AddDays(–1) |where {$_.Gettype().Name-eq "VmRemovedEvent"} |Sort CreatedTime -Descending |Select CreatedTime, UserName,FullformattedMessage |Export-Csv -encoding utf8 -Path SEUDIRETORIO\vmsremovidas.csv
